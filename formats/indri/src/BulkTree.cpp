...
#include "indri-platform.h"
#include <assert.h>
#include <vector>
#include "File.hpp"
#include "delete_range.hpp"
...
indri::file::BulkTreeWriter::BulkTreeWriter() : _write(_file, 1024 * 1024)
{
  _blockID = 0;
...
indri::file::BulkTreeReader::BulkTreeReader(File &file) : _file(&file),
                                                          _ownFile(false),
                                                          _head(0),
                                                          _tail(0)
{
...
indri::file::BulkTreeReader::BulkTreeReader(File &file, UINT64 length) : _file(&file),
                                                                         _ownFile(false),
                                                                         _fileLength(length),
                                                                         _head(0),
                                                                         _tail(0)
...
indri::file::BulkTreeReader::BulkTreeReader() : _ownFile(false),
                                                _file(0),
                                                _head(0),
                                                _tail(0)
{
...
indri::file::BulkTreeIterator::BulkTreeIterator(File &file) : _file(file)
{
  _pairIndex = -1;
  _blockIndex = 0;
  _fileLength = 0;
...
indri::file::BulkTreeIterator::BulkTreeIterator(File &file, UINT64 whichBlock, int whichPair) : _file(file)
{
