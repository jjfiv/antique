...
pub fn decompress_document(value: ValueEntry) -> Result<Document, Error> {
    ...
    debug_assert_eq!(text_size, text.as_bytes().len());
...
mod tests {
    ...
    fn test_decompress() {
        let raw_data: &[u8] = &[
            ...
            // compat-version
            0, 0, 0, 1, 
            0, 0, 0, 198, 200, 1, 32, 0, 0,
            0, 17, 0, 0, 0, 120, 0, 9, 1, 240, 60, 3, 0, 0, 0, 43, 47, 104, 111, 109, 101, 47, 106,
            102, 111, 108, 101, 121, 47, 97, 110, 116, 105, 113, 117, 101, 47, 100, 97, 116, 97,
            47, 105, 110, 112, 117, 116, 115, 47, 82, 69, 65, 68, 77, 69, 46, 116, 120, 116, 0, 0,
            0, 1, 0, 0, 0, 5, 116, 105, 116, 108, 101, 9, 67, 8, 0, 116, 84, 1, 13, 240, 110, 58,
            32, 80, 114, 105, 100, 101, 32, 97, 110, 100, 32, 80, 114, 101, 106, 117, 100, 105, 99,
            101, 10, 65, 117, 116, 104, 111, 114, 58, 32, 74, 97, 110, 101, 32, 65, 117, 115, 116,
            101, 110, 10, 67, 104, 97, 112, 116, 101, 114, 115, 58, 32, 49, 45, 53, 10, 83, 111,
            117, 114, 99, 101, 58, 32, 104, 116, 116, 112, 58, 47, 47, 119, 119, 119, 46, 103, 117,
            116, 101, 110, 98, 101, 114, 103, 46, 111, 114, 103, 47, 102, 105, 108, 101, 115, 47,
            49, 51, 52, 50, 47, 49, 51, 52, 50, 45, 48, 46, 116, 120, 116, 10,
        ];
        assert!(raw_data.starts_with(SNAPPY_HEADER));

        let mut snappy = Decoder::new();
        let decompressed = snappy.decompress_vec(&raw_data[SNAPPY_HEADER.len()..]).unwrap();
